...
grep -iEv 'CCTV|卫视|凤凰|星空|channel v|channel-v|动画|动漫|少儿|儿童|卡通|炫动|baby|disney' tmp.list >> output.list

echo '动画,#genre#' >> output.list
grep -iE '动画|动漫|少儿|儿童|卡通|炫动|baby|disney' tmp.list >> output.list
